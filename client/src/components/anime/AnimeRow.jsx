...
const AnimeRow = React.memo(({ title, genre, data, loading: externalLoading }) => {
 ...
 className="text-2xl md:text-3xl font-bold text-white group-hover/row:text-white transition-colors cursor-pointer"
 >
 {title}
 </h2>
 <span
 onClick={() => navigate(`/library?genre=${genre || 'All'}`)}
 className="text-sm font-semibold text-netflixGray hover:text-white cursor-pointer transition-colors tracking-wide uppercase"
 >
 Explore all
 </span>
 </div>
 ...
 <button
 onClick={() => slide('left')}
 className="absolute left-0 top-2 bottom-8 md:bottom-12 z-40 bg-white/10 w-12 hidden md:group-hover/arrows:flex items-center justify-center text-h2 hover:bg-neutral-900/90 transition-all text-white"
 >
 ...
 <div
 ref={rowRef}
 className="flex gap-4 md:gap-6 overflow-x-auto scroll-smooth px-6 md:px-16 no-scrollbar netflix-row pb-8 md:pb-12 pt-2"
 >
 ...
 <button
 onClick={() => slide('right')}
 className="absolute right-0 top-2 bottom-8 md:bottom-12 z-40 bg-white/10 w-12 hidden md:group-hover/arrows:flex items-center justify-center text-h2 hover:bg-neutral-900/90 transition-all text-white"
 >
