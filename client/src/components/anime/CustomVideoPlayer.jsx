...
const CustomVideoPlayer = React.forwardRef(({ src, onTimeUpdate, onEnded, onLoadedData, onError, initialTime = 0, onPlay, skipTimes, skipAnim, topControls, children, anime, episode, onBack, onEpisodesClick, onToggleFullscreen }, ref) => {
 ...
 <button onClick={handlePlayPause} className="text-white hover:text-white transition-colors drop-shadow-lg flex-shrink-0 rounded-xl" title={isPlaying ?"Pause" :"Play"}>
 {isPlaying ? <BoxyPause size={28} fill="currentColor" /> : <BoxyPlay size={28} fill="currentColor" />}
 </button>
 
 <div className="flex items-center gap-2 md:gap-3 flex-shrink-0">
 <button onClick={(e) => skipBy(-10, e)} className="text-white hover:text-white transition-transform active:scale-90 drop-shadow-lg relative flex items-center justify-center" title="Rewind 10s">
 <BoxyRotateCcw size={28} />
 <span className="text-[8px] font-medium absolute mt-0.5">10</span>
 </button>
 <button onClick={(e) => skipBy(10, e)} className="text-white hover:text-white transition-transform active:scale-90 drop-shadow-lg relative flex items-center justify-center" title="Forward 10s">
 ...
 <button onClick={toggleMute} className="text-white hover:text-white transition-colors drop-shadow-lg rounded-xl">
 ...
 <span className="text-netflixGray">{formatTime(duration)}</span>
 </div>
 </div>

 {/* Next Episode, Episodes, Fullscreen */}
 <div className="flex items-center gap-4 md:gap-7 flex-shrink-0">
 <button className="text-white hover:text-white transition-colors flex items-center gap-2 drop-shadow-lg rounded-xl" title="Next Episode">
 <BoxySkipForward size={26} fill="currentColor" /> 
 </button>
 {onEpisodesClick && !isFullscreen && (
 <button onClick={onEpisodesClick} className="text-white hover:text-white transition-colors drop-shadow-lg rounded-xl" title="Episodes">
 <BoxyEpisodes size={26} />
 </button>
 )}
 <button onClick={toggleFullscreen} className="text-white hover:text-white transition-colors drop-shadow-lg rounded-xl">
