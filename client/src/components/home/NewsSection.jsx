...
const NewsSection = React.memo(() => {
 ...
 <div className="mb-12">
 <h2 className="text-h3 md:text-h1 font-semibold text-white">News</h2>
 <div className="h-4" />
 <p className="text-micro md:text-h4 font-medium text-white">Stay updated with the latest in anime.</p>
 </div>
 ...
 />
 <div className="text-netflixGray text-center px-2 absolute inset-0 flex flex-col items-center justify-center -z-0">
 <BoxyMessage size={24} className="mx-auto mb-1 opacity-50" />
 <p className="text-[10px] font-medium uppercase text-balance">toady-news.svg</p>
 </div>
 </div>

 {/* Text Content */}
 <div className="flex flex-col items-center md:items-start max-w-sm">
 <h3 className="text-h4 md:text-h3 font-medium text-white mb-2">All caught up!</h3>
 <p className="text-micro font-normal text-netflixGray leading-relaxed">
 ...
 <div>
 <h2 className="text-h3 md:text-h1 font-semibold text-white leading-none">Breaking news</h2>
 </div>
 <button onClick={() => navigate('/library')} className="text-[13px] font-medium text-white hover:text-white transition-all underline decoration-1 underline-offset-8">
 View all
 </button>
 </div>
 ...
 <h3 className="text-h3 md:text-h2 font-semibold text-white mb-4 leading-[1.1] transition-colors">
 {news[0]?.title}
 </h3>
 <p className="text-white text-micro font-medium">{formatDate(news[0]?.date)}</p>
 </div>
 </motion.div>
 ...
 <h4 className="text-body md:text-h4 font-medium text-white group-hover:text-white transition-colors line-clamp-2 leading-tight">
 {item.title}
 </h4>
 <p className="text-white text-[11px] font-medium mt-1">{formatDate(item.date)}</p>
 </div>
 </motion.div>
 ))}
 </div>
 ...
 <div>
 <h5 className="text-h4 font-medium text-white mb-1">Join the community</h5>
 <p className="text-[12px] text-white leading-relaxed font-medium max-w-sm">
 Get the latest anime news and seasonal previews.
 </p>
 </div>
 <button className="w-full md:w-auto px-8 py-3.5 bg-white text-black text-micro font-medium hover:bg-white/90 transition-all active:scale-95 whitespace-nowrap rounded-xl">
 Subscribe
 </button>
 </div>
 </div>
