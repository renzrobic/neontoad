...
const CommentsDrawer = ({ reelId, reelAuthorId, onClose, onUpdateCount }) => {
 ...
 <button onClick={onClose} className="w-8 h-8 flex items-center justify-center text-white hover:text-white hover:bg-white/10 backdrop-blur-md rounded-xl rounded-full transition-all">
 <BoxyX size={18} />
 </button>
 </div>
 ...
 {c.userId === reelAuthorId && <span className="text-[9px] font-medium bg-white/10 backdrop-blur-md rounded-xl text-white px-1.5 py-0.5 rounded-full">OP</span>}
 <span className="text-[10px] text-white font-medium">{c.createdAt?.toDate ? formatDistanceToNow(c.createdAt.toDate(), { addSuffix: true }) : 'Just now'}</span>
 </div>
 <p
 className={`text-[13px] font-medium leading-relaxed ${c.isSpoiler && !revealedSpoilers.has(c.id) ? 'blur-sm cursor-pointer select-none text-white' : 'text-white/85'}`}
 ...
 className="text-[11px] font-semibold text-white hover:text-white mt-1 transition-colors">Reply</button>
 </div>
 <button onClick={() => toggleLikeComment(c.id, c.likedBy)} className={`pt-1 flex-shrink-0 transition-all ${c.likedBy?.includes(auth.currentUser?.uid) ? 'text-white' : 'text-white/25 hover:text-white'}`}>
 <BoxyHeart size={14} fill={c.likedBy?.includes(auth.currentUser?.uid) ? 'currentColor' : 'none'} />
 <span className="text-[10px] font-medium block text-center mt-0.5">{c.likes || 0}</span>
 </button>
 </div>
 ...
 <p className={`text-[12px] font-medium leading-relaxed ${r.isSpoiler && !revealedSpoilers.has(r.id) ? 'blur-sm cursor-pointer select-none text-white' : 'text-white'}`}
 ...
 <div className="h-full flex flex-col items-center justify-center text-white gap-3 pt-16">
 <BoxyMessage size={36} className="opacity-20" />
 <p className="text-[12px] font-medium text-white">No comments yet. Start the conversation!</p>
 </div>
 )}
 </div>
 ...
 <span className="text-[11px] font-medium text-white">Replying to <span className="text-white font-semibold">@{replyingTo.userName}</span></span>
 <button onClick={() => setReplyingTo(null)} className="text-white hover:text-white transition-colors">
 <BoxyX size={14} />
 </button>
 </div>
 )}
 ...
 className="flex-grow bg-transparent text-[13px] font-medium text-white focus:outline-none placeholder:text-white min-w-0"
 />
 <button
 type="button" onClick={() => setIsSpoiler(!isSpoiler)}
 className={`flex-shrink-0 text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded-full transition-all ${isSpoiler ? 'bg-neutral-700 text-white' : 'text-white hover:text-white'}`}
 >
 {isSpoiler ? '⚠ Spoiler' : 'Spoiler'}
 </button>
 <button type="submit" disabled={!newComment.trim()}
 className="flex-shrink-0 text-[12px] font-medium text-white disabled:opacity-25 hover:text-white transition-colors rounded-xl">
 Post
 </button>
 </form>
 </div>
