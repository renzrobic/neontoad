...
const ReelCard = React.memo(({ video, isActive, isAdjacent, onUploadClick, isMuted, setIsMuted, volume, setVolume, isDesktop }) => {
 ...
 <span className="text-[12px] font-medium text-white drop-shadow-md">{likesCount > 999 ? `${(likesCount / 1000).toFixed(1)}K` : likesCount}</span>
 </button>
 ...
 <span className="text-[12px] font-medium text-white drop-shadow-md">{commentsCount}</span>
 </button>
 ...
 >
 <button onClick={e => { e.stopPropagation(); handleReport(); setShowMoreMenu(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-[13px] font-medium text-white hover:bg-white/10 backdrop-blur-md rounded-xl transition-colors text-left">
 <BoxyShield size={16} /> Report
 </button>
 {video.animeId && (
 <button onClick={e => { e.stopPropagation(); navigate(`/watch/${video.animeId}`); setShowMoreMenu(false); }} className="w-full flex items-center gap-3 px-4 py-3 text-[13px] font-medium text-white hover:bg-white/10 backdrop-blur-md rounded-xl transition-colors text-left">
 ...
 <BoxyTV size={14} className="text-white flex-shrink-0 mr-2 group-hover:text-white transition-colors z-10" />
 {isLong ? (
 ...
 <div className="whitespace-nowrap text-[12px] font-medium text-white group-hover:text-white transition-colors flex w-max" style={{ animation: `marquee ${Math.max(video.animeTitle.length * 0.2, 5)}s linear infinite` }}>
 <div className="flex items-center gap-4 pe-4"><span>{video.animeTitle}</span><span className="text-white">•</span></div>
 <div className="flex items-center gap-4 pe-4"><span>{video.animeTitle}</span><span className="text-white">•</span></div>
 </div>
 </div>
 ) : (
 <span className="whitespace-nowrap text-[12px] font-medium text-white group-hover:text-white transition-colors">{video.animeTitle}</span>
 )}
 </div>
 );
 };
 ...
 <button onClick={() => { setIsMuted(m => !m); showControlsTemporarily(); }} className="text-white hover:text-white transition-colors flex-shrink-0">
 ...
 <button onClick={() => navigate('/')} className="w-12 h-11 flex items-center justify-center bg-white/8 backdrop-blur-md hover:bg-white/15 text-white hover:text-white transition-all rounded-full"><BoxyTV size={20} /></button>
 <button onClick={onUploadClick} className="w-12 h-11 flex items-center justify-center bg-white/8 backdrop-blur-md hover:bg-white/15 text-white hover:text-white transition-all rounded-full"><BoxyTV size={20} /></button>
 </div>
 ...
 <span className="text-white font-semibold text-[14px]">{displayName}</span>
 {user && video.userId !== user?.uid && (
 <><span className="text-white font-medium mx-1">•</span><button onClick={e => { e.stopPropagation(); toggleFollow(video.userId); }} className={`text-[14px] font-medium transition-all ${isFollowing ? 'text-white' : 'text-white hover:text-white'}`}>{isFollowing ? 'Following' : 'Follow'}</button></>
 )}
 </div>
 <p onClick={e => { e.stopPropagation(); setIsDescExpanded(p => !p); }} className={`text-[12px] text-white font-medium leading-relaxed cursor-pointer ${isDescExpanded ? '' : 'line-clamp-2'}`}>{video.caption}</p>
 </div>
 ...
 <div className="flex items-center gap-2 mb-2"><span className="text-[11px] font-medium text-white drop-shadow-md">{formatTime(currentTime)}</span><span className="text-[11px] font-medium text-white drop-shadow-md">/ {formatTime(duration)}</span></div>
 ...
 <div className="absolute right-4 pointer-events-auto"><button onClick={e => { e.stopPropagation(); onUploadClick(); }} className="w-10 h-10 flex items-center justify-center text-white hover:text-white transition-colors"><BoxyTV size={24} /></button></div>
 </div>
 <AnimatePresence>
 {showControls && (
 ...
 <div className="flex items-center gap-2 mb-1.5 flex-wrap"><img loading="lazy" src={displayAvatar} className="w-9 h-9 rounded-full object-cover border-2 flex-shrink-0" alt="" /><span className="text-white font-medium text-[14px]">{displayName}</span>{user && video.userId !== user?.uid && (<><span className="text-white font-medium mx-0.5">•</span><button onClick={() => toggleFollow(video.userId)} className={`text-[14px] font-medium transition-all ${isFollowing ? 'text-white' : 'text-white hover:text-white'}`}>{isFollowing ? 'Following' : 'Follow'}</button></>)}</div>
 <p onClick={() => setIsDescExpanded(p => !p)} className={`text-[13px] text-white font-medium leading-relaxed cursor-pointer ${isDescExpanded ? '' : 'line-clamp-2'}`}>{video.caption}</p>
 </div>
 ...
 <span className="text-[12px] font-medium text-white drop-shadow-md">{formatTime(currentTime)} <span className="text-white">/ {formatTime(duration)}</span></span>
 </div>
 </div>
