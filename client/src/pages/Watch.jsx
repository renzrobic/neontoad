...
const EpisodesWheel = ({ episodes, initialIndex, onClose, onSelect, anime }) => {
 ...
 <button onClick={onClose} className="w-10 h-10 rounded-full bg-white/5 backdrop-blur-md flex items-center justify-center text-white hover:text-white hover:bg-white/10 transition-all border border-white/10">
 <BoxyX size={24} />
 </button>
 </div>
 ...
 <h3 className="text-white text-xl md:text-2xl font-medium mb-1 truncate drop-shadow-md">Ep {ep.episodeNumber}</h3>
 <p className="text-white text-xs md:text-sm line-clamp-1 drop-shadow-md">{ep.title}</p>
 </div>
 {distance === 0 && (
 ...
 <div className="absolute bottom-8 text-netflixGray text-xs font-medium pointer-events-none z-[120] text-center w-full px-4 drop-shadow-md">
...
const Watch = () => {
 ...
 <h2 className="text-h3 md:text-h2 font-medium text-white mb-4">Screen limit reached</h2>
 <p className="text-netflixGray mb-10 leading-relaxed text-micro md:text-[15px] font-normal">
 ...
 className="bg-primary text-black font-medium py-4 rounded-md w-full hover:scale-105 active:scale-95 transition-transform text-micro"
 >
 Switch Profile
 </button>
 <button 
 onClick={() => navigate('/')}
 className="bg-transparent border-2 border-white/10 text-white font-medium py-4 rounded-md w-full hover:bg-white/5 active:scale-95 transition-all text-micro"
 >
 Return Home
 </button>
 </div>
 ...
 >
 <BoxyChevron direction="left" size={32} className="text-white group-hover:text-white transition-colors" />
 </button>
 <div className="flex flex-col gap-0.5 min-w-0 drop-shadow-lg">
 <span className="text-[10px] md:text-micro font-medium text-white tracking-wider uppercase truncate">{anime?.title || 'Series'}</span>
 <h1 className="text-body md:text-h4 font-medium text-white leading-snug md:leading-none line-clamp-2 md:truncate">
 Episode {episode.episodeNumber} <span className="text-white mx-1 md:mx-2 font-normal">—</span> {episode.title}
 </h1>
 </div>
 </div>
 ...
 className="hidden md:flex items-center gap-2 text-white font-medium hover:text-white transition-all drop-shadow-lg text-micro uppercase"
 >
 ...
 className={`w-10 h-10 flex items-center justify-center transition-all hover:scale-110 drop-shadow-lg ${showDrawer ? 'text-netflixGray' : 'text-white hover:text-white'}`}
 >
 <BoxyList size={24} />
 </button>
 <button 
 onClick={() => { navigator.clipboard.writeText(window.location.href); toast.success('Link copied'); }}
 className="hidden md:flex w-10 h-10 items-center justify-center text-white hover:text-white transition-all drop-shadow-lg hover:scale-110"
 >
 <BoxyShare size={22} />
 </button>
 </div>
